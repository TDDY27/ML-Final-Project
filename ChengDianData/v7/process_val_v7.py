...
for t in range(int(sys.argv[1]),int(sys.argv[2])):
    ...
    val_b['home_team_win'] = 1 - val_b['home_team_win']
    for c in diff_list:
        val_b['diff_'+c] = -val_b['diff_'+c]
    for c in ratio_list:
        val_b['ratio_'+c] = -val_b['ratio_'+c]
    for name in name_list:
        val['team_is_'+name] = val['home_team_is_'+name].copy()
        val_b['team_is_'+name] = val_b['away_team_is_'+name].copy()
    for team_season in team_season_list:
        val['team_season_is_'+team_season] = val['home_team_season_is_'+team_season].copy()
        val_b['team_season_is_'+team_season] = val_b['away_team_season_is_'+team_season].copy()
    val = pd.concat([val,val_b],axis = 0)
    for name in name_list:
        val = val.drop(['home_team_is_'+name], axis = 1)
        val = val.drop(['away_team_is_'+name], axis = 1)
    for team_season in team_season_list:
        val = val.drop(['home_team_season_is_'+team_season], axis = 1)
        val = val.drop(['away_team_season_is_'+team_season], axis = 1)
    
    data.to_csv("./datav7_t{}/raw_vdata_full_column.csv".format(t), index=False)
    val.to_csv("./datav7_t{}/raw_val_full_column.csv".format(t), index=False)
    ...
    data.to_csv("./datav7_t{}/norm_vdata_full_column.csv".format(t), index=False)
    val.to_csv("./datav7_t{}/norm_val_full_column.csv".format(t), index=False)
    std_data.to_csv("./datav7_t{}/std_vdata_full_column.csv".format(t), index=False)
    std_val.to_csv("./datav7_t{}/std_val_full_column.csv".format(t), index=False)