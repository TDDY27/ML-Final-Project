...
with open('./datav3_t{}/std_data.csv'.format(t), newline='') as csvfile:
    rows = csv.reader(csvfile)
    for r in rows:
        all_data.append(r)

with open('./datav3_t{}/std_vdata.csv'.format(t), newline='') as csvfile:
    rows = csv.reader(csvfile)
    for r in rows:
        v_data.append(r)

with open('./datav3_t{}/std_val.csv'.format(t), newline='') as csvfile:
    rows = csv.reader(csvfile)
    for r in rows:
        val.append(r)

with open('./datav3_t{}/std_test1.csv'.format(t), newline='') as csvfile:
    rows = csv.reader(csvfile)
    for r in rows:
        test.append(r)
