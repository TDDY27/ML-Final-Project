...
with open('./c3_raw_data_full_column.csv', newline='') as csvfile:
    rows = csv.reader(csvfile)
    for r in rows:
        all_data.append(r)

with open('./c3_raw_vdata_full_column.csv', newline='') as csvfile:
    rows = csv.reader(csvfile)
    for r in rows:
        v_data.append(r)

with open('./c3_raw_val_full_column.csv', newline='') as csvfile:
    rows = csv.reader(csvfile)
    for r in rows:
        val.append(r)

with open('./c3_raw_test1_full_column.csv', newline='') as csvfile:
    rows = csv.reader(csvfile)
    for r in rows:
        test.append(r)
